...
fn on_level_loaded(
    ...
    let script =
        scripting.new_script_from_module_id(script_assets.level_1.id(), ExecutionMode::Async)?;
    ...
    let collision_margin =
        if let Ok(v) = runtime.get_struct_value_as_number(&script_module, "collision_margin") {
            v.as_real() as f32
        } else {
            cube_gap / 4.0
        };
    ...
    let half_size =
        if let Ok(v) = runtime.get_struct_value_as_number(&script_module, "half_side_length") {
            v.as_uns() as i32
        } else {
            6
        };

    let rigid_body = if let Ok(v) = runtime.get_struct_value(&script_module, "static")
        && v.as_bool()
    {
        RigidBody::Static
    } else {
    ...
    let with_synth = if let Ok(v) = runtime.get_struct_value(&script_module, "with_synth") {
        v.as_bool()
    } else {
        true
    };
